...
def eval_class(gt_annos,
    ...
    if len(gt_annos) != len(dt_annos):
        print("ground truth len:", len(gt_annos), "result len:", len(dt_annos))
    assert len(gt_annos) == len(dt_annos)
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None, overlap_logger=None):
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            # if float(min_overlaps[i, 0, j])==0.7 and float(min_overlaps[i, 1, j])==0.7 and float(min_overlaps[i, 2, j])==0.7:
            #     moderate_list = []
            #     moderate_list.append(mAP3d_R40[j, 1, i])
            #     moderate_list.append(mAPbev_R40[j, 1, i])
